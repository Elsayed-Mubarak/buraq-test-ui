...
type Props = {
    ...
    setOpenEditView: React.Dispatch<React.SetStateAction<boolean>>
}

export default function EditViewModal({ openEditView, setOpenEditView }: Props) {
    const createViewRef = useOutsideClick(() => setOpenEditView(false))
    if (!openEditView) return null;
    return (
        <Portal>
            <div className="fixed z-[999] flex items-center justify-center top-0 left-0 right-0 bottom-0 w-full h-full bg-[rgba(0,0,0,0.16)]">
                <div ref={createViewRef} className="bg-white max-w-[750px] w-full rounded-xl relative">
                    <div className="flex items-center justify-between py-5 px-6 border-b border-primary-50">
                        <div className="text-[#3a3a3a] text-xl font-semibold">Edit View</div>
                        <button onClick={() => setOpenEditView(false)}>
                            <span>
                    ...
                    <div className="py-4 px-6 border-b border-primary-50">
                        ...
                        <div className="grid items-center gap-4 text-sm text-secondary-50 min-h-[45px] grid-cols-[100px_155px_160px_199px]">
                    ...
                    <div className="py-4 px-6 flex items-center justify-end">
                        <ReusableButton onClick={() => { }} disabled={true}>Edit view</ReusableButton>
                    </div>
                </div>
            </div>
        </Portal>
