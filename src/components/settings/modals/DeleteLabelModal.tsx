...
type Props = {
    ...
    selectedLabel: any,
    setSelectedLabel: React.Dispatch<React.SetStateAction<any>>
    deleteLabel: (labelId: string) => void,
    isPending: boolean
}

export default function DeleteLabelModal({ isOpen, setIsOpen, selectedLabel, setSelectedLabel, deleteLabel, isPending }: Props) {
            ...
            <div role="dialog"
                ...
                <div ref={containerRef} className='w-[360px] bg-white rounded-xl'>
                    <div className='py-5 px-6 flex items-center justify-between border-b border-[#e4e4e4]'>
                        <div className='text-[#3a3a3a] text-xl font-semibold'>Delete Label</div>
                        <span onClick={handleClose}>
                            <X className='h-6 w-6 cursor-pointer text-[#6d6d6d]' />
                        </span>
                    </div>
                    <div className="p-6 border-b border-primary-50">
                        <div className='text-[13px]'>Are you sure you want to delete the label titled '{selectedLabel?.name ?? "UnKown"}' ?</div>
                    </div>
                    <div className="flex justify-end py-5 px-6 items-center">
                        <div className='flex items-center gap-3'>
                            ...
                            <ReusableButton className='bg-[#f00] hover:bg-[#ed2b2b]' onClick={() => deleteLabel(selectedLabel._id)} disabled={isPending}>
