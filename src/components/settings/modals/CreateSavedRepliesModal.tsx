...
export default function CreateSavedRepliesModal({ isOpen, setIsOpen }: Props) {
    const queryClient = useQueryClient();
    const containerRef = useOutsideClick(() => handleCloseModal())
    const [name, setName] = useState<string>('')
    const [reply, setReply] = useState<string>('')
    const [file, setFile] = useState<File | null>(null)
    const [nameRrror, setNameError] = useState(false)
    const [replyError, setReplyError] = useState(false)
    const inputRef = useRef<HTMLInputElement>(null)


    const { mutate: createSavedReply, isPending } = useMutation({
    ...
    function addSavedReply() {
        ...
        createSavedReply({ title: name, reply });
    }
            ...
            <div role="dialog"
                ...
                <div ref={containerRef} className='w-[360px] bg-white rounded-xl'>
                    <div className='py-5 px-6 flex items-center justify-between border-b border-[#e4e4e4]'>
                        <div className='text-[#3a3a3a] text-xl font-semibold'>Create Saved Reply</div>
                    ...
                    <div className="p-6 border-b border-primary-50">
                        <div className="group mb-3">
                            <Label className={`group-focus-within:text-primary-500 text-sm text-secondary-50 block w-full mb-1 ${nameRrror ? 'text-[#f00]' : ''}`}>Name</Label>
                            <input
                                value={name}
                                ...
                                className={`p-2 hover:border-primary-500 text-sm block w-full rounded-lg border border-[#e4e4e4] focus:outline-none focus:border-primary-500 ${nameRrror ? 'border-[#f00]' : ''}`} type="text" />
                    ...
                    <div className="flex justify-end py-5 px-6 items-center">
                        <ResuableButton onClick={addSavedReply} disabled={isPending || !name || !reply}>
