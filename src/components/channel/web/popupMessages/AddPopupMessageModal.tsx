...
import { createPopupMessage } from "@/service/popupMessagesServices";
...
export default function AddPopupMessageModal({ isOpen, setIsOpen }: Props) {
    ...
    const { mutate: addPopupMessage, isPending } = useMutation({
        mutationFn: createPopupMessage,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["popupMessages"] })
            setIsOpen(false)
            toast.success("Pop-up message created successfully")
            setUrl("")
            setMessage("")
            setFileName("")
            setMediaFile(null)
    ...
    function handleCreatePopupMessage() {
            ...
            <div className="fixed z-[999] flex items-center justify-center top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.16)]">
                <div ref={containerRef} className="bg-white w-[360px] rounded-xl">
                    <div className="flex items-center justify-between border-b border-primary-50 py-5 px-6">
                        <div className="text-[#3a3a3a] text-xl font-semibold">Create Pop-up Message</div>
                        <span>
                    ...
                    <div className="py-5 px-6">
                        ...
                        <div className="mb-4">
                            ...
                            <div className="h-[160px] relative border border-primary-50 rounded-lg">
                                ...
                                <div className="bg-[#f3f3f3] p-2 h-[40px] rounded-t-lg flex items-center justify-between">
                                    <span>B</span>
                                    <span onClick={() => setOpenEmojiPicker(prev => !prev)} className="cursor-pointer select-none">
                                        <Smile size={20} />
                                    </span>
                                </div>
                                <textarea value={message} onChange={(e) => setMessage(e.target.value)} className="text-sm text-secondary-50 py-2 px-3 h-[118px] rounded-b-lg  w-full border-none outline-none focus:outline-none resize-none bg-white"></textarea>
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="h-[64px] flex items-center p-3 w-full border bg-[#f8f8f8] border-[#e4e4e4] rounded-lg">
                                ...
                                {!mediaFile && <label htmlFor="widgetIcon" className="w-full h-full cursor-pointer flex items-center gap-5">
                                ...
                                {mediaFile && <div className="w-full h-full text-sm flex items-center justify-between gap-2">
                    ...
                    <div className="py-5 px-6 flex items-center justify-end border-t border-primary-50">
                        <ReusableButton disabled={!url || isPending} onClick={handleCreatePopupMessage}>Create</ReusableButton>
                    </div>
                </div>
            </div>
        </Portal>
