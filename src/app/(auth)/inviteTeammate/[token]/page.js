...
export default function Page() {
    ...
    const { isPending, mutate: addTeamateFn } = useMutation({
        mutationFn: (data) => addTeamate(token, data),
        onSuccess: () => {
                    ...
                    <div className="rounded-[20px] border border-primary-50 p-10">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className="group relative mb-7">
                                ...
                                <label
                                    className={`sibling mb-1 block text-sm font-semibold group-focus-within:text-primary-500 ${errors?.email?.message?.length > 0 ? "text-[#f00]" : "text-secondary-50"}`}
                                    htmlFor="email"
                                >
                                    Business Email
                                </label>
                                <input
                                    className={`block w-full rounded-md border p-2 text-black hover:border-primary-600 ${errors?.email?.message?.length > 0 ? "border-[#f00]" : "border-primary-50"}`}
                                    type="email"
                                    {...register("email", {
                                        required: "This field is required",
                                        pattern: {
                            ...
                            <div className="group relative mb-7">
                                ...
                                <label
                                    className={`sibling mb-1 block text-sm font-semibold group-focus-within:text-primary-500 ${errors?.password?.message?.length > 0 ? "text-[#f00]" : "text-secondary-50"}`}
                                    htmlFor="password"
                                >
                                    Password
                                </label>
                                ...
                                <div className="relative">
                                    <input
                                        className={`block w-full rounded-md border p-2 text-black hover:border-primary-600 ${errors?.password?.message?.length > 0 ? "border-[#f00]" : "border-primary-50"}`}
